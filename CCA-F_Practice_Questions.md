...
- **a)** <code>temperature=0</code> — the model produces its most confident output, best for professional copy
- **b)** <code>temperature=0.2</code> — mostly deterministic but allows slight variation for freshness
- **c)** <code>temperature=0.8</code> — high variability produces genuinely different taglines from the same input, which is what "variations" requires
- **d)** Temperature doesn't matter — you'd write 5 different prompts, not vary temperature
...
- **a)** No — temperature controls variability across runs, not correctness within a single run; a wrong answer at <code>temperature=0</code> is just consistently wrong
- **b)** Yes — deterministic output eliminates hallucination and improves accuracy
- **c)** Yes — but only if combined with <code>top_p=0.5</code> for full effect
- **d)** No — but setting <code>temperature=1.5</code> (above the normal range) would improve accuracy
...
- **a)** Claude misinterpreted the task and thought it was complete
- **b)** <code>stop_sequences</code> requires a period at the end (<code>"END."</code>) to work correctly
- **c)** <code>stop_sequences</code> performs literal substring matching — the letters E-N-D appear inside "ends", triggering the cut. Stop sequence strings should be unlikely to appear naturally in normal output
- **d)** The token limit was set too low, and "END" is a common closing token
...
- **a)** Diminishing returns kick in after 3-5 examples, while token cost grows linearly — 40 examples costs 8× more with minimal accuracy gain over 5 well-chosen ones
- **b)** Accuracy scales linearly with example count, so 40 examples is roughly 8× better than 5
- **c)** The API rejects prompts with more than 10 few-shot examples
- **d)** More examples always help; the only cost is response latency
...
- **a)** They're the same feature with different names — Anthropic renamed CoT to extended thinking
- **b)** CoT is a prompting technique (adding "think step-by-step" to your prompt); extended thinking is a model mode enabled via API request that produces internal reasoning tokens in separate ThinkingBlocks
- **c)** CoT only works on Sonnet models; extended thinking only works on Opus models
- **d)** CoT is for math problems; extended thinking is for coding tasks — different task types
...
- **a)** Instruct Claude in the system prompt: "Always start your response with Diagnosis:"
- **b)** Use <code>stop_sequences=["Diagnosis:"]</code> to force the format
- **c)** Prefill the assistant response with "Diagnosis: " — Claude's response literally starts from that anchor and continues naturally
- **d)** Set <code>response_format={"type": "prefix", "value": "Diagnosis:"}</code>
...
- **b)** The comparison should be case-insensitive: `"Tool_Result"`
- **c)** `stop_reason` isn't populated on the first call — only on the final call
- **d)** The correct value when Claude requests a tool is `"tool_use"`, not `"tool_result"`
...
- **c)** Only when using extended thinking mode
- **d)** Yes — `response.content` is a list and may contain multiple blocks of different types
...
- **a)** £0.30 — cost is capped once the context window is filled
- **b)** £0.60 — twice as many turns, twice the cost
- **c)** Significantly more than £0.60 — later turns send more history than earlier ones
...
- **b)** Claude's reply is never appended to `messages`, so on each call Claude only sees user turns
- **c)** The API call should include `remember=True` for multi-turn
- **d)** The loop needs `max_tokens` set explicitly to preserve history
...
- **a)** 20 — one user and one assistant per turn
- **b)** 10 — one per user query
...
- **c)** Claude compresses everything past 500 tokens into a summary
- **d)** `max_tokens` limits Claude's REPLY length; it doesn't affect input, memory, or history at all
...
- **a)** Summarise older turns and replace them in the messages list — preserving key facts, cutting token count
- **b)** Delete random messages from the middle to reduce length
- **c)** Reduce `max_tokens` to lower per-turn response cost
...
- **a)** The context window is unlimited on paid Enterprise tiers
...
- **d)** The window is available, but every token in the request costs input tokens — a 150K conversation is genuinely expensive on every call
...
- **b)** First is a dry-run for cost estimation; second is the real call
- **c)** First returns Claude's request to use the tool; developer executes it; second sends the result back for Claude's final answer
...
- **a)** An empty list — Claude signals tool use through `stop_reason` only
- **b)** A ToolUseBlock with the tool `name`, `input` arguments, and a unique `id`
...
- **a)** `user` — anything sent TO Claude has role `user`, regardless of content type
- **b)** `tool` — a special role for tool results
- **c)** `system` — tool results are system-level information
- **d)** `assistant` — because Claude is the one 'assisting' with the tool
...
- **b)** Only the first result — Claude re-requests the others in follow-up calls
- **c)** One `assistant` message with all three results concatenated as JSON text
- **d)** One `user` message whose content is a list of three `tool_result` blocks, each with the matching `tool_use_id`
...
- **a)** Continue the loop: while `stop_reason == "tool_use"`, run the tool and send result back. Only exit when `stop_reason == "end_turn"`
- **b)** Batch all tool calls upfront — Claude can't make sequential decisions
- **c)** Reset the messages list between tools to avoid cross-contamination
...
- **b)** The API needs a `max_iterations` parameter set explicitly
- **c)** Claude has an infinite-generation bug — set `stop_sequences` to prevent it
- **d)** The loop code isn't appending the assistant response before the tool_result, so Claude re-issues the same request each iteration
...
- **c)** Read the error, understand it as failure, and translate it into user-appropriate language (apology, alternatives)
- **d)** Ignore the error and fabricate an answer from training data
...
- **a)** In the API client library — it should auto-convert
- **b)** Inside the tool function itself — one fix protects every caller
...
- **b)** MCP decouples tool implementation from AI application — one server (e.g., GitHub) can be reused across many AI apps without duplicating code
- **c)** MCP tools bypass the tool_use loop, reducing API calls per session
...
- **b)** MCP servers must now be hosted centrally on Linux Foundation infrastructure
- **c)** MCP is an industry-standard protocol maintained across multiple vendors — skills transfer across platforms, not vendor-locked
...
- **b)** stdio for personal-machine / local tools; HTTP for remote or shared / hosted servers
- **c)** HTTP for personal machine; stdio for enterprise
- **d)** stdio only for testing; HTTP always in production
...
- **b)** Caches the return value across calls
- **c)** Converts the function to run asynchronously
- **d)** Exposes the function as an MCP tool, auto-generating the input schema from type hints and using the docstring as the tool description
...
- **c)** Nothing — Claude Desktop auto-discovers all running MCP servers
...
- **b)** A command-line / IDE agent tool from Anthropic that lets developers use Claude as an agent for real coding tasks (edit files, run commands, use MCP tools)
- **c)** A subscription tier on anthropic.com
- **d)** A public dataset used to train Claude on code
...
- **a)** Claude Code runs it but logs it for later review
- **b)** Claude Code refuses to run any file-modifying commands ever
- **c)** Claude Code shows the command and waits for your explicit approval before executing
...
- **Q45: a** — Unhandled exceptions kill the Python process before the second API call can happen. Claude never receives the tool_result and therefore never has a chance to respond. The user sees whatever their chat client shows for a stalled request — often nothing, or a hang, or a client-side error. This is the entire reason error handling in tool functions matters: no try/except means no fallback because Claude is never contacted. The 'auto-retries' option assumes Claude has retry logic — it doesn't unless explicitly programmed. The 'answers from training data' option assumes Claude has some fallback — again, only if the API call actually happens. The 'apologises with alternatives' option is what happens IF Claude receives an error string as tool_result, not with an unhandled exception.
...
- **Q49: b** — Encapsulate the type contract at the source: `str()` inside the tool function protects every call site. If you fix it externally at each call site, you have to remember to apply the conversion every time you use the function — inevitable that you'll miss one and get inconsistent behaviour or an API error. The 'client library auto-converts' option fabricates behaviour the SDK doesn't have. The 'faster execution' option misses the actual reason (maintenance safety, not performance). The 'doesn't matter' option is wrong — it matters significantly for long-term maintenance and consistency.
